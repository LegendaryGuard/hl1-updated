...
void DLLEXPORT CAM_Think()
{
	...
	*/
	...
	if (player && (gEngfuncs.GetMaxClients() > 0) && (gViewPort && gEngfuncs.pEventAPI))
	{
		...
		Vector camera_offset = Vector(VIEW_XOFF, VIEW_YOFF, VIEW_ZOFF);
		Vector player_origin = player->origin + camera_offset + view_ofs;
		Vector camera_origin = player_origin - camForward * 100; // + offset; 
