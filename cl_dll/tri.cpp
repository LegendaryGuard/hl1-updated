...
Vector v_up, v_right, v_forward; //Defined out here so other areas of code can grab them
...
* @param Rect size: the size of the new crosshair
* @param int k: if have no clue what those do , added for compatibility with the engine function
* @param int l: see k
* @param int m: see k
*/
void SetCrosshair(HSPRITE sprite, Rect size, int k, int l, int m)
{
...
{
	//calculate positions
	Vector circle[EXPLOSION_LAYERS + 1][36];
...
void Draw_WorldSprite(HSPRITE sprite, Vector pos, float size, int RenderMode)
{
	Vector temporg;
